...
if ticker_input:
    ...
    if st.button("📈 Run Analysis"):
        with st.spinner("Fetching data and computing portfolio..."):
            ...
            st.write(f"• Mean Daily Log Return: {daily_mean:.5f} ({daily_mean * 100:.2f}%)")
            st.write(f"• Mean Monthly Log Return: {monthly_mean_return:.5f} ({monthly_mean_return * 100:.2f}%)")
            st.write(f"• Mean Yearly Log Return: {yearly_mean_return:.5f} ({yearly_mean_return * 100:.2f}%)")
